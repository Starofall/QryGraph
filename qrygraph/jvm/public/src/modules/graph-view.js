(function(Dataflow) {
    ...
    '<div class="dataflow-graph-panzoom">'+
      '<div class="dataflow-graph zoom-normal">'+
        '<div class="dataflow-edges">'+
          '<svg class="dataflow-svg-edges" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns="http://www.w3.org/2000/svg" width="1800" height="1800"></svg>'+
        '</div>'+
  ...
  Graph.View = Backbone.View.extend({
    ...
    addNode: function(node){
      ...
      // var CustomType = this.model.dataflow.nodes["base-resizable"];
    ...
    startHighlightCompatible: function (port, fromInput) {
      this.model.nodes.each(function (node) {
        node.outputs.each(function (output) {
          ...
          if (output.canConnect(port) && (port.get('type') == 'all' || output.get('type') === 'all' || output.get('type') === port.get('type'))) {
            return;
          }
          output.view.blur();
        });
        node.inputs.each(function (input) {
          ...
          if (input.canConnect(port) && (port.get('type') == 'all' || input.get('type') === 'all' || input.get('type') === port.get('type'))) {
            return;
          }
          input.view.blur();
        });
