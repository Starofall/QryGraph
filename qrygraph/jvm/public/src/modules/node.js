...
( function (Dataflow) {
    ...
    Node.Model = Backbone.Model.extend({
        ...
        initialize: function () {
            ...
            // here we parse the resultType from the options
            this.resultTypes = this.get("resultTypes");

            var self = this;
            // this.set("selected", true)
            // self.selected = this.get("selected");
        ...
        setState: function (name, value) {
            ...
            state[name] = value;
            if (this["input" + name]) {
                this["input" + name](value, this, name);
            }
            _.defer(function () {
                UIListener().updateFieldValue(name, value);
            });
            this.trigger("change:state", name, value); //TODO: design this
        },
        setBang: function (name) {
            if (this["input" + name]) {
                this["input" + name]("bang", this);
            }
        ...
        recieve: function (name, value, port) {
            //console.log("recieve",name,value,this)
            // The listener is set up in Edge/initialize
            if (typeof this["input" + name] === "function") {
                this["input" + name](value, this, port);
            } else {
                this["_" + name] = value;
            }
        },
        ...
        toJSON: function () {
            return {
                ...
                resultTypes: this.get("resultTypes"),
                type: this.get("type"),
                x: this.get("x"),
                y: this.get("y"),
