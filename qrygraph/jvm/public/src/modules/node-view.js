( function (Dataflow) {
    ...
    var headerTemplate =
        ...
        '<% if (type!="Output") { %><a class="deleteButton btn btn-xs" style="height: 18px;padding: 0px;width: 20px;"><i class="fa fa-remove fa-fw" style="color: darkred" aria-hidden="true"/></a><% } %>' +
        '</div>' +
    ...
    var template =
        ...
        '</div>';
        // +
        // '<div class="dataflow-node-inner"><% print(resultTypes) %></div>';
    ...
    Node.View = Backbone.View.extend({
        ...
        initialize: function (options) {
            ...
            this.$(".deleteButton").on("click", function () {
                UIListener().removeNode(self.model.id)
                console.log("DELETE ON:", self.model.id)
            });

            // as we want to overwrite the default selection behaviour, we trigger a changed for the next tick
            _.defer(function () {
        ...
        moveToPosition: function (x, y) {
            ...
            var self = this
            _.defer(function () {
                UIListener().nodeMovedEvent(self.model.id, x, y)
            })
        },
        ...
        select: function (event, deselectOthers) {
            UIListener().nodeActivated(this.model.id)
            // Don't click graph
            if (event) {
                event.stopPropagation();
            }
