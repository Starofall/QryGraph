( function (Dataflow) {
    ...
    Input.View = Backbone.View.extend({
        ...
        initialize: function (options) {
            ...
            if (nodeState && nodeState[this.model.id] && nodeState[this.model.id] != "") {
            ...
            this.model.parentNode.on('change:state', function () {
                var state = this.model.parentNode.get('state');
                if (!state || state[this.model.id] === undefined || state[this.model.id] === "" || state[this.model.id] === null) {
        ...
        changeEdgeStart: function (event, ui) {
            ...
            if (this.isConnected) {
                ...
                if (changeEdge) {
                    // Remove edge
                    _.defer(function () {
                        UIListener().removeEdge(changeEdge.source.id, changeEdge.target.id)
                    })
                    //hack to remove the small edge point on kill
                    $(".dataflow-port-plug").hide()
                    changeEdge.remove();
                    return
                    // Make preview
                    if (ui) {
        ...
        connectEdge: function (event, ui) {
            ...
            if (!this.model.canConnect(otherPort)) {
            ...
            var self = this
            _.defer(function () {
                UIListener().createEdge(otherPort.parentNode.id, self.model.parentNode.id, otherPort.id, self.model.id)
            })
        ...
        holePosition: function () {
            ...
            if (!this._holePosition) {
                ...
                var left = graph.get("panX") + node.get("x") + 08;
                var top = graph.get("panY") + node.get("y") + 38 + index * 20;
